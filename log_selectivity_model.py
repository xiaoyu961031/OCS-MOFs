...
# Load the data
file_path = 'log_selectivity_data.csv'  # Replace with your file path
...
X_train_combined = train_data_combined.drop(columns=['filename', 'log']).select_dtypes(include=[np.number])
y_train_combined = train_data_combined['log']

# Preparing the DB13 test set
X_test_db13 = test_data_db13.drop(columns=['filename', 'log']).select_dtypes(include=[np.number])
y_test_db13 = test_data_db13['log']
...
scatter_ax.set_xticks([0, 1, 2, 3])
scatter_ax.set_yticks([0, 1, 2, 3])
...
plt.savefig('true_vs_predicted_plot_logselectivity.png')
# Display the plot
plt.show()
